...
class UserManager(BaseUserManager):
    def create_user(self, email, password=None):
        if not email:
            raise ValueError('ユーザー登録にはmailアドレスが必要です')
...
class Profile(models.Model):
    ...
    department = models.CharField(max_length=100, blank=True, null=True, verbose_name="部　　署")
    phone_number = models.IntegerField(blank=True, null=True, verbose_name="携帯番号")
    gender = models.CharField(max_length=1, choices=GENDER_CHOICE, default=None, verbose_name="性　　別", blank=True, null=True)
    birthday = models.DateField(blank=True, null=True, verbose_name="生年月日（YYYY-MM-DD）")
